<div class="flex flex-col">
                            ...
                            <tr>
                                ...
                                <th scope="col"
                                    ...
                                    <div class="flex text-left">
                                        Nombre del evento
                                        @if ($sortAsc)
                                            <span class="cursor-pointer" wire:click="sortBy('nombre_evento')">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                                    viewBox="0 0 20 20" fill="currentColor">
                                                    <path
                                                        d="M3 3a1 1 0 000 2h11a1 1 0 100-2H3zM3 7a1 1 0 000 2h7a1 1 0 100-2H3zM3 11a1 1 0 100 2h4a1 1 0 100-2H3zM15 8a1 1 0 10-2 0v5.586l-1.293-1.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L15 13.586V8z" />
                        ...
                        @foreach ($eventos as $evento)
                            <tr>
                                ...
                                <td class="flex justify-items-center px-6 py-4 text-sm font-medium text-right
                                        whitespace-nowrap">
                                    <button wire:click="confirmEventoEdit({{ $evento->id }})"
                                    ...
                                    <button wire:click="triggerConfirm({{ $evento->id }})"
    ...
    {{-- Modal para agregar una nuevo evento --}}
    <x-jet-dialog-modal wire:model="confirmEventoAdd">
        <x-slot name="title">
            {{ __('Agregar nuevo evento') }}
        </x-slot>

        <x-slot name="content">
            {{ __('Ingresa el nombre de nuevo evento') }}

            <div class="mt-4">
                <input type="text" wire:model="nombre_evento" name="nombre_evento" id="nombre_evento"
        ...
        <x-slot name="footer">
            <x-jet-secondary-button wire:click="$set('confirmEventoAdd', false)" wire:loading.attr="disabled">
                {{ __('Cancelar') }}
    ...
    <x-jet-dialog-modal wire:model="confirmEventoEdit">
        <x-slot name="title">
            {{ __('Actualizar evento') }}
        </x-slot>

        <x-slot name="content">
            {{ __('Ingresa el nuevo nombre del evento') }}
        ...
        <x-slot name="footer">
            <x-jet-secondary-button wire:click="$set('confirmEventoEdit', false)" wire:loading.attr="disabled">
                {{ __('Cancelar') }}
            </x-jet-secondary-button>
