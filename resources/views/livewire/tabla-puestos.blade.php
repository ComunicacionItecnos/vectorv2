<div class="flex flex-col">
        ...
        <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-2 m-2">
                <div class="col-span-1 sm:col-span-1 flex justify-start items-center">
                    <label for="especialidad_puesto" class="block text-3xl font-medium text-gray-700">Puestos</label>
                </div>
                <div class="col-span-1 sm:col-span-1 flex justify-end items-center">
                    <button wire:click="confirmPuestoAdd"
                            ...
                            <tr>
                                ...
                                <th scope="col"
                                    ...
                                    <div class="flex text-left">
                                        Nivel
                                        @if ($sortAsc)
                                            <span class="cursor-pointer" wire:click="sortBy('especialidad_puesto')">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                                    viewBox="0 0 20 20" fill="currentColor">
                                                    <path
                                                        d="M3 3a1 1 0 000 2h11a1 1 0 100-2H3zM3 7a1 1 0 000 2h7a1 1 0 100-2H3zM3 11a1 1 0 100 2h4a1 1 0 100-2H3zM15 8a1 1 0 10-2 0v5.586l-1.293-1.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L15 13.586V8z" />
                        ...
                        @foreach ($puestos as $puesto)
                            <tr>
                                ...
                                <td class="flex justify-items-center px-6 py-4 text-sm font-medium text-right
                                        whitespace-nowrap">
                                    <button wire:click="confirmPuestoEdit({{ $puesto->id }})"
                                    ...
                                    <button wire:click="triggerConfirm({{ $puesto->id }})"
    ...
    {{-- Modal para agregar una nuevo puesto --}}
    <x-jet-dialog-modal wire:model="confirmPuestoAdd">
        <x-slot name="title">
            {{ __('Agregar un nuevo puesto') }}
        </x-slot>

        <x-slot name="content">

            <div class="mt-4">
                <label for="nivel_id" class="block text-sm font-medium text-gray-700">Nivel</label>
                <select id="nivel_id" wire:model="nivel_id" name="nivel_id"
                    class=" block w-full px-3 py-2 mt-1 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    <option></option>
                    @foreach ($niveles as $nivel)
                        <option value="{{ $nivel->id }}">{{ $nivel->nombre_nivel }}
                        </option>
                    @endforeach
                </select>
                @error('nivel_id')
            ...
            <div class="mt-4">
                <label for="especialidad_puesto" class="block text-sm font-medium text-gray-700">Especialidad
                    puesto</label>
                <input type="text" wire:model="especialidad_puesto" name="especialidad_puesto" id="especialidad_puesto"
                    class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                @error('especialidad_puesto')
                    <p class="mt-1 mb-1 text-xs text-red-600 italic">
                        {{ $message }}
                    </p>
                @enderror
        ...
        <x-slot name="footer">
            <x-jet-secondary-button wire:click="$set('confirmPuestoAdd', false)" wire:loading.attr="disabled">
                {{ __('Cancelar') }}
    ...
    {{-- Modal para editar un puesto existente --}}

    <x-jet-dialog-modal wire:model="confirmPuestoEdit">
        <x-slot name="title">
            {{ __('Editar puesto') }}
        </x-slot>
        ...
        <x-slot name="footer">
            <x-jet-secondary-button wire:click="$set('confirmPuestoEdit', false)" wire:loading.attr="disabled">
                {{ __('Cancelar') }}
            </x-jet-secondary-button>

            <x-jet-button class="ml-2" dusk="confirm-password-button" wire:click="guardar"
                wire:loading.attr="disabled">
                {{ __('Actualizar') }}
            </x-jet-button>
        </x-slot>
